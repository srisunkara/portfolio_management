...
class HoldingCRUD(BaseCRUD[HoldingDtl]):
    ...
    def list_all(self) -> List[HoldingDtl]:
        rows = pg_db_conn_manager.fetch_data(
            "SELECT holding_id, holding_dt, portfolio_id, security_id, quantity, price, market_value, created_ts, last_updated_ts "
    ...
    def save(self, item: HoldingDtlInput) -> HoldingDtl:
        ...
        INSERT INTO holding_dtl (
            holding_id, holding_dt, portfolio_id, security_id, quantity, price, market_value, created_ts, last_updated_ts
        ...
        params = (
            ...
            h.quantity, h.price, h.market_value, h.created_ts, h.last_updated_ts
        )
    ...
    def update(self, pk: int, item: HoldingDtlInput) -> HoldingDtl:
        ...
        params = (
            ...
            item.price, item.market_value, date_utils.get_current_date_time(), pk
        )
        affected = pg_db_conn_manager.execute_query(sql, params)
        if affected == 0:
