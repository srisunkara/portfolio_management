...
export default function SecurityForm({ mode }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        // Build form fields for create/edit from SecurityDtl (skip date/time + readOnly)
        const f = formFieldsForAction("SecurityDtl", isEdit ? "edit" : "create");
        let initial = {};

        if (isEdit) {
          const data = await api.getSecurity(id);
          initial = {};
          ...
          for (const field of f) {
            initial[field.name] = defaultValueForType(field.type);
          }
            // Ensure default currency is USD on create (user can change it)
          if (initial.security_currency == null || initial.security_currency === "") {
            initial.security_currency = "USD";
          }
        ...
        if (alive) setError("Failed to load security data.");
      } finally {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.updateSecurity(id, payload);
      } else {
        await api.createSecurity(payload);
      }
      navigate("/securities", { replace: true });
    } catch (e) {
      setError("Failed to save security.");
  ...
  return (
    <div>
      <h1 style={{ marginTop: 0 }}>{isEdit ? "Edit Security" : "Add Security"}</h1>
