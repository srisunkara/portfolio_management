...
export default function TradingPlatformForm({ mode }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const f = formFieldsForAction("ExternalPlatformDtl", isEdit ? "edit" : "create");
        let initial = {};
        if (isEdit) {
          const data = await api.getExternalPlatform(id);
        ...
        if (alive) setError("Failed to load external platform data.");
      } finally {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.updateExternalPlatform(id, payload);
      } else {
        await api.createExternalPlatform(payload);
      }
      navigate("/external-platforms", { replace: true });
    } catch (e) {
      setError("Failed to save external platform.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <h1 style={{ marginTop: 0 }}>{isEdit ? "Edit External Platform" : "Add External Platform"}</h1>
